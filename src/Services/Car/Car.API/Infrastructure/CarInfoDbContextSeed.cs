        ...
        if (!context.CarCharacteristics.Any())
        {
            ...
            var characteristics = csvParser.Parse<CarCharacteristic>(
                ...
                values => new CarCharacteristic
                {
                    ...
                    Name = values[1],
                    ParentId = int.TryParse(values[2], out var parentId) ? parentId : default,
                    DateCreate = uint.TryParse(values[3], out var dateCreate) ? dateCreate : default,
                    DateUpdate = uint.TryParse(values[4], out var dateUpdate) ? dateUpdate : default,
        ...
        if (!context.CarCharacteristicValues.Any())
        {
            ...
            var characteristicValues = csvParser.Parse<CarCharacteristicValue>(
                ...
                values => new CarCharacteristicValue
                {
                    ...
                    Value = values[1],
                    Unit = values[2],
                    CarCharacteristicId = int.TryParse(values[3], out var characteristicId) ? characteristicId : default,
                    CarModificationId = int.Parse(values[4]),
                    DateCreate = uint.TryParse(values[5], out var dateCreate) ? dateCreate : default,
                    DateUpdate = uint.TryParse(values[6], out var dateUpdate) ? dateUpdate : default,
        ...
        if (!context.CarEquipment.Any())
        {
            ...
            var equipment = csvParser.Parse<CarEquipment>(
                ...
                values => new CarEquipment
                {
                    Id = int.Parse(values[0]),
                    Name = values[1],
                    DateCreate = uint.TryParse(values[2], out var dateCreate) ? dateCreate : default,
                    DateUpdate = uint.TryParse(values[3], out var dateUpdate) ? dateUpdate : default,
                    CarModificationId = int.Parse(values[4]),
                    PriceMin = int.TryParse(values[5], out var priceMin) ? priceMin : default,
                    CarTypeId = int.Parse(values[6]),
                    Year = int.TryParse(values[7], out var year) ? year : default,
                });
        ...
        if (!context.CarModifications.Any())
        {
            ...
            var modifications = csvParser.Parse<CarModification>(
                ...
                values => new CarModification
                {
                    ...
                    CarModelId = int.Parse(values[2]),
                    Name = values[3],
                    DateCreate = uint.TryParse(values[4], out var dateCreate) ? dateCreate : default,
                    DateUpdate = uint.TryParse(values[5], out var dateUpdate) ? dateUpdate : default,
                    CarTypeId = int.Parse(values[6]),
                    CarEngineTypeId = int.TryParse(values[7], out var engineType) ? engineType : default,
                    CarDriveTypeId = int.TryParse(values[8], out var driveType) ? driveType : default,
                    CarTransmissionTypeId = int.TryParse(values[9], out var transmissionType) ? transmissionType : default,
                    EngineCapacity = int.TryParse(values[10], out var engineCapacity) ? engineCapacity : default,
                    EnginePower = int.TryParse(values[11], out var enginePower) ? enginePower : default,
                    FuelConsumptionCombined = decimal.TryParse(values[12], out var fuelConsumption) ? fuelConsumption : default,
                    GroundClearance = decimal.TryParse(values[13], out var groundClearance) ? groundClearance : default,
                });
        ...
        if (!context.CarSeries.Any())
        {
            ...
            var series = csvParser.Parse(
                ...
                values => new CarSerie
                {
                    ...
                    DateCreate = uint.TryParse(values[3], out var dateCreate) ? dateCreate : default,
                    DateUpdate = uint.TryParse(values[4], out var dateUpdate) ? dateUpdate : default,
                    CarGenerationId = int.TryParse(values[5], out var generationId) ? generationId : default,
                    CarTypeId = int.Parse(values[6])
                });
